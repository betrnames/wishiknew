...
function buildMeme(regret: string, forceLifeHack: boolean, theme: ThemeKey): Meme {
  ...
  if (/planner|plan|organize|productivity/.test(normalized)) {
    ...
    hackDetail =
      "Pick one list (Notes, Google Tasks, or paper). Add only todayâ€™s top 3. No new apps until day 8."
    ...
    hack = "Default to â€˜let me check and get back to you.â€™"
    hackDetail =
      "Wait 10 minutes before accepting. If you still want to go, reply with a hard end time (â€˜I can do 7â€“8:30â€™)."
    ...
    hackDetail =
      "Leave hard ones on the counter 2â€“3 days, then fridge once soft. Eat within 24 hours of â€˜ready,â€™ or mash + lemon and seal tight."
    ...
    hackDetail =
      "In Zoom/Teams settings: mute mic on join, stop video on join. Turn them on only when youâ€™re ready."
    ...
    hack = "Only text ETA after youâ€™re in motion."
    hackDetail =
      "Shoes on, keys in hand, door open â€” then send â€˜leaving now, ~X minâ€™ with a real number from Maps."
    ...
    hackDetail =
      "On desktop: click the recipient field and scan the list. If itâ€™s a long thread, Reply (not Reply all) unless everyone must act."
  } else if (/episode|netflix|autoplay|binge|one more/.test(normalized)) {
    title = "Autoplay Betrayal"
    subtitle = "The â€˜continueâ€™ button had a better plan than your sleep did."
    hack = "Turn off autoplay + set a hard stop."
    hackDetail =
      "Netflix/YouTube: disable autoplay. Decide â€˜one episodeâ€™ before you sit down; put the remote/phone across the room when itâ€™s done."
    ...
    title = "A Fragrant Mistake"
    subtitle = "The office will remember this longer than your name."
    hack = "Prep two â€˜no-thinkâ€™ lunches on Sunday."
    hackDetail =
    ...
    hackDetail =
      "Add to cart, close the tab, revisit tomorrow. If itâ€™s still useful and in budget, buy once â€” not three â€˜deals.â€™"
  } else if (/password|login|account|hacked|2fa/.test(normalized)) {
    title = "Security Theater"
    subtitle = "Your password was â€˜password123â€™ with confidence."
    ...
    hackDetail =
      "Draft the angry version in Notes (not email). Wait 30â€“60 minutes, then send a short, factual message with one clear ask."
    ...
    hack = "Donâ€™t text feelings after 10pm."
    hackDetail =
      "Sleep on it. If it still matters, say it once in person or a calm daytime message â€” no essay threads."
  } else {
    ...
    hackDetail =
      "Write: â€˜Next time Iâ€™ll ___ before I ___.â€™ Put that line in your phone notes and read it when the urge returns."
  }
  ...
  return {
    ...
    explanation: practical
      ? "A practical â€˜wish I knewâ€™ â€” the punchline is funny because the fix is boring and real."
      : "An emotional plot twist. Distance helps; a tiny rule helps more.",
    lifeHack: lifeHack ? hack ?? "Write one rule youâ€™d tell a friend in your situation." : undefined,
    lifeHackVersion: lifeHack
      ? hackDetail ?? "Keep the rule under 15 words so youâ€™ll actually remember it next time."
      : undefined,
    theme,
    createdAt: Date.now(),
  }
...
export function MemeLab() {
  ...
  function sendToWreckSh() {
    if (!meme) return
    // No account link yet â€” queue only; never attaches a username
    showNotice(
      anonymous
        ? "Queued without your name (demo). Sharing elsewhere is still your choice."
        : "Queued as a public card (demo). Prefer private? Turn on â€˜Keep private on this site.â€™",
    )
  }
  ...
  async function copyText() {
    ...
    showNotice("Share text copied â€” no raw confession included")
  }
  ...
  async function shareCard() {
    ...
    try {
      if (navigator.share) {
        ...
        showNotice("Shared â€” your social account is separate from this site")
        return
      }
    } catch {
    ...
    showNotice("Opened X with the card text â€” not your private input")
  }
        ...
        <section className="grid items-start gap-10 pt-10 lg:grid-cols-[0.9fr_1.1fr] lg:gap-20 lg:pt-16">
          <div className="max-w-xl">
            ...
            <p className="mt-6 max-w-md text-lg leading-7 text-muted-foreground">
              Drop the thing you wish you could un-send. Weâ€™ll make it funny enough to survive â€”
              ...
              <CardContent className="p-5 sm:p-6">
                ...
                <div className="mt-4 flex flex-col gap-3 border-t pt-4 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <Button onClick={generate} disabled={!canGenerate} className="w-full gap-2 font-semibold sm:w-auto">
                    ...
                    {isGenerating ? "Cookingâ€¦" : "Make it a meme"}
            ...
            <div className="mt-3 flex flex-col gap-2 text-xs text-muted-foreground sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
              <span>âŒ˜ + Enter Â· History stays on this device</span>
              <button
                type="button"
                ...
                title={
                  anonymous
                    ? "We donâ€™t attach your name on this site. Sharing a card later is still your choice."
                    : "Public on this site means the card isnâ€™t marked private â€” social posts still use your own accounts."
                }
              >
            ...
            <p className="mt-1.5 text-[11px] leading-4 text-muted-foreground/80">
              Private â‰  invisible forever. Download or share posts the <span className="font-medium text-muted-foreground">card</span>, not
            ...
            {meme && (
              <div className="mt-4 space-y-3">
                ...
                >
                  ...
                  Queue to wreck.sh (no name Â· demo)
                </Button>
              </div>
            )}
          </div>
